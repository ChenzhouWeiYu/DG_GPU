...
template<uInt OrderU=3, uInt OrderP=OrderU-1, typename Flux = AirFluxD, 
        typename GaussQuadCell = GaussLegendreTet::Auto, 
        typename GaussQuadFace = GaussLegendreTri::Auto>
class InCompressibleImplicitDiffusion {
private:
    // 基函数空间定义
    ...
    DenseMatrix<3,3> compute_stress_kernel(
                 const vector3f& left_vec,  // ∇ϕ_j 或 n
                 const vector3f& right_vec // ∇ϕ_i
                ) const;
    Scalar m_mu;

public:
    InCompressibleImplicitDiffusion(Scalar mu = 1e-2):m_mu(mu){};
