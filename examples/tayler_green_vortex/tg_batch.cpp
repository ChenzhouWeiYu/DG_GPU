...
constexpr Scalar param_mu = 1e-2;
...
inline Type u_xyz(Type x, Type y, Type z, Type t){
    ...
    return u*std::exp(-8*M_PI*M_PI*param_mu*t);
}
template<typename Type>
...
inline Type fe_xyz(Type x, Type y, Type z, Type t){
    ...
    Scalar exp24 = std::exp(-24*M_PI*M_PI*t*param_mu) * M_PI/(param_gamma-1);
    Scalar Cos24 = cos2x*cos2y*(cos4x-cos4y);
    Scalar exp16 = -4*std::exp(-16*M_PI*M_PI*t*param_mu)*M_PI*M_PI*param_mu;
    Scalar Cos16 = 1+cos4x*cos4y;
...
void Run(uInt N){
    ...
    for(uInt kkkk=0;kkkk<100;kkkk++){    
        ...
        do{
            ...
            Scalar h = 1.2/N * 0.5773502691896257645091487805019574556476 / 3.0,   vel = 0.0;
            for(uInt fid=0; fid<cmesh.m_faces.size(); ++fid) {
