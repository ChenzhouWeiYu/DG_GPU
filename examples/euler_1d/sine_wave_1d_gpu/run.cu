...
#define Expand_For_Flux(Order) {\
    if(FluxType=="LFNOLimiter") RunCompressibleEuler<Order,LF75C,false>(meshN, fsm, logger, 0b00); \
    if(FluxType=="RoeNOLimiter") RunCompressibleEuler<Order,Roe75C,false>(meshN, fsm, logger, 0b00); \
    if(FluxType=="HLLNOLimiter") RunCompressibleEuler<Order,HLL75C,false>(meshN, fsm, logger, 0b00); \
    if(FluxType=="HLLCNOLimiter") RunCompressibleEuler<Order,HLLC75C,false>(meshN, fsm, logger, 0b00); \
    if(FluxType=="HLLEMNOLimiter") RunCompressibleEuler<Order,HLLEM75C,false>(meshN, fsm, logger, 0b00); \
