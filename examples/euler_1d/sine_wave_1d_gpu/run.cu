...
#include "dg/dg_flux/physical_flux/euler_flux.h"
#include "dg/dg_flux/euler_physical_flux.h"
#include "dg/dg_flux/numerical_flux/hll_flux.h"
...
template<uInt Order, uInt N_states, bool OnlyNeigbAvg = false>
...
int main(int argc, char** argv){
    ...
    if(order == 1) RunCompressibleEuler<1,5,false>(meshN, fsm, logger, 0b00);
    if(order == 2) RunCompressibleEuler<2,5,false>(meshN, fsm, logger, 0b01);
    if(order == 3) RunCompressibleEuler<3,5,false>(meshN, fsm, logger, 0b01);
...
template<uInt Order, uInt N_states, bool OnlyNeigbAvg>
void RunCompressibleEuler(uInt N, FilesystemManager& fsm, LoggerSystem& logger, uInt limiter_flag){
    ...
    **   设置物理参数
    \* ======================================================= */
    Thermodynamics<0> thermo(1.4);
    EulerFlux<0> phys_flux(thermo);
    HLLFlux<0> num_flux(thermo, phys_flux);
    Scalar param_gamma = thermo.gamma;
    ...
    **   算子 和 限制器 的实例化
    \* ======================================================= */
    ExplicitConvectionGPU<Basis::OrderBasis,5> convection(phys_flux, num_flux);
    ...
    **   设置初值
    ...
    const auto& primitive = phys_flux.thermo.reconstruct({0.88893668, 0.88893668, 0.0, 0.0, 0.88893668*2.94446834});
    const auto& pflux = phys_flux.compute(primitive);
    print(std::array<Scalar,9>{primitive.rho,primitive.u,primitive.v,primitive.w,
        primitive.p,primitive.e,primitive.T,primitive.a,primitive.gamma_eff});
    print(pflux);
    print(HLL75C::computeFlux({0.88893668, 0.88893668, 0.0, 0.0, 0.88893668*2.94446834}));

    print(num_flux.compute(
        {0.88893668, 0.88893668, 0.0, 0.0, 0.88893668*2.94446834}, 
        {0.88893668, 0.88893668, 0.0, 0.0, 0.88893668*2.94446834},
        {1.0,3.2,1.75}));
    print(HLL75C::computeNumericalFlux(
        {0.88893668, 0.88893668, 0.0, 0.0, 0.88893668*2.94446834},
        {0.88893668, 0.88893668, 0.0, 0.0, 0.88893668*2.94446834}, 
        {1.0,3.2,1.75}));
