...
#define Expand_For_Flux(Order) {\
    if(FluxType=="LF") RunCompressibleEuler<Order,LF75C>(meshN, fsm, logger); \
    if(FluxType=="Roe") RunCompressibleEuler<Order,Roe75C>(meshN, fsm, logger); \
    if(FluxType=="HLL") RunCompressibleEuler<Order,HLL75C>(meshN, fsm, logger); \
    if(FluxType=="HLLC") RunCompressibleEuler<Order,HLLC75C>(meshN, fsm, logger);\
    if(FluxType=="HLLEM") RunCompressibleEuler<Order,HLLEM75C>(meshN, fsm, logger);\
}
