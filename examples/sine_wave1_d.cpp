...
void Run(uInt N){
    ...
    GeneralMesh mesh = OrthHexMesh({0.0, 0.0, 0.0},{2*M_PI, 2*M_PI/N, 2*M_PI/N},{N,1,1});
    ...
    for(uInt fid=0;fid<cmesh.m_faces.size();fid++){           
        if(cmesh.m_faces[fid].m_neighbor_cells[1]==uInt(-1)){ 
            ...
            if(std::abs(face.m_normal[1])>0.5 || std::abs(face.m_normal[2])>0.5 )                
