...
void Run(uInt N, FilesystemManager fsm){
    ...
    **   间隔 Dt 时间保存一次
    ...
    for(uInt save_step = 0; save_step < 40; save_step++){    
        ...
        Scalar cfl = 0.5; 
        Scalar h = 3.0/(N*N);
        Scalar dt_convection = cfl * h / (1.0 + param_Air_c);
        Scalar dt_diffusion = cfl * h*h / (1.0/param_Re);  // h^2 / ν
        Scalar max_dt = Dt*std::min(dt_convection, dt_diffusion) * std::pow(Order,-2);
        // max_dt = Dt * 4 * std::pow(3.0/(N*N),2) * std::pow(Order,-2);
        // max_dt = 8*max_dt; 
        ...
        do{ 
            ...
            **   需要判断是否超过 Dt，超过了就截断
            ...
            for(uInt picard_iter = 0; picard_iter < 100; picard_iter++){
                ...
                /* 用显格式推进 */
                LongVector<DoFs> U_k_tmp = U_k + dt / mass * (rhs - sparse_mat.multiply(U_k));
                ...
                logging("Re-discretize, calculate nonlinear residuals");
                {
                    ...
                    BlockSparseMatrix<DoFs,DoFs> sparse_mat;
                    get_matrix(sparse_mat, rhs, U_k);
                    const auto& residual = rhs - sparse_mat.multiply(U_k);
                    std::ostringstream oss;
                ...
                if(picard_iter>10 || delta < 1e-8 || rel_delta < 1e-8 || (picard_iter>10 && rate_delta > 0.99)) break;
            }
