...
TimeIntegrator<DoFs, Order>::TimeIntegrator(
    const DeviceMesh& mesh,
    LongVectorDevice<DoFs>& U_n,
    const LongVectorDevice<DoFs>& r_mass,
    Scalar CFL)
    : mesh_(mesh), U_n_(U_n), r_mass_(r_mass), CFL_(CFL)
{
    // 初始化内部缓存（大小与 U_n 一致）
...
template<uInt DoFs, uInt Order>
void TimeIntegrator<DoFs, Order>::set_scheme(TimeIntegrationScheme scheme) {
    scheme_ = scheme;
...
// template<uInt DoFs, uInt Order>
// void TimeIntegrator<DoFs, Order>::advance(
//     ExplicitConvectionGPU<Order>& convection,
//     Scalar curr_time,
//     Scalar dt)
// {
//     const int num_blocks = (DoFs + 31) / 32;
...
//             rk_stage<<<num_blocks, 32>>>(U_n_.d_blocks, U_n_.d_blocks, U_1_.d_blocks, dt, DoFs);
//             break;
//         }

//         case TimeIntegrationScheme::SSP_RK3: {
//             // Stage 1: U_1 = U_n - dt * R(U_n)
//             U_1_.fill_with_scalar(0.0);
//             convection.eval(mesh_, U_n_, U_1_, curr_time);
//             rk_stage<<<num_blocks, 32>>>(U_1_.d_blocks, U_n_.d_blocks, U_1_.d_blocks, dt, DoFs);
//             cudaDeviceSynchronize();

//             // Stage 2: U_2 = 3/4 U_n + 1/4 U_1 - 1/4 dt * R(U_1)
//             U_2_.fill_with_scalar(0.0);
//             convection.eval(mesh_, U_1_, U_2_, curr_time + dt);
//             rk_stage_mixed<<<num_blocks, 32>>>(U_2_.d_blocks, U_n_.d_blocks, U_1_.d_blocks, r_mass_.d_blocks, dt, 0.75, 0.25, 0.25, DoFs);
//             cudaDeviceSynchronize();

//             // Stage 3: U_n = 1/3 U_n + 2/3 U_2 - 2/3 dt * R(U_2)
//             U_temp_.fill_with_scalar(0.0);
//             convection.eval(mesh_, U_2_, U_temp_, curr_time + 0.5*dt);
//             rk_stage_mixed<<<num_blocks, 32>>>(U_n_.d_blocks, U_n_.d_blocks, U_2_.d_blocks, r_mass_.d_blocks, dt, 1.0/3.0, 2.0/3.0, 2.0/3.0, DoFs);
//             cudaDeviceSynchronize();
//             break;
//         }
