...
ComputingMesh create_mesh(uInt N){
    ...
    std::vector<std::array<double, 2>> points = {{0.0, -0.5}, {2.0, -0.5}, {2.0, 0.5}, {0.0, 0.5}};
    std::vector<std::array<double, 2>> internal_points;

    for(uInt ny = 2; ny < 10*N - 1; ny++){
        Scalar y = -0.5 + 2*dy * ny;
        for(uInt nx = 0; nx < 20*N - 3; nx++){
            Scalar x = nx * 2*dx;
            if(x>2.0-h) break;
            internal_points.push_back({x,y});
        }
    }
    for(uInt ny = 2; ny < 10*N - 2; ny++){
        Scalar y = -0.5 + 2*dy * ny + dy;
        for(uInt nx = 0; nx < 20*N - 3; nx++){
            Scalar x = nx * 2*dx + dx;
            if(x>2.0-h) break;
            internal_points.push_back({x,y});
