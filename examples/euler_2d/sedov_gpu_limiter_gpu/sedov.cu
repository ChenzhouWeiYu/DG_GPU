...
void Run(uInt N, FilesystemManager& fsm, LoggerSystem& logger){
    ...
    **   正交基，只需要计算、保存对角元。  r_mass 表示是 倒数
    ...
    while (total_time < final_time) {
        ...
        positivelimiter.constructMinMax(gpu_U_n);
        // print(CPUpositivelimiter.per_cell_min - positivelimiter.d_per_cell_min.download());
        // print(CPUpositivelimiter.per_cell_max - positivelimiter.d_per_cell_max.download());
        // const auto& h_per_cell_max = positivelimiter.d_per_cell_max.download_to_host();
        // const auto& h_per_cell_min = positivelimiter.d_per_cell_min.download_to_host();
        // print(h_per_cell_min);
        // print(h_per_cell_max);
        // U_n = gpu_U_n.download();
        // printf("更新前 %.6e    ", U_n.dot(U_n));
