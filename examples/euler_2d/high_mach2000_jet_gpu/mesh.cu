...
ComputingMesh create_mesh_half(uInt N);
ComputingMesh create_mesh_full(uInt N);
ComputingMesh create_mesh_unstructure(uInt N);

ComputingMesh create_mesh(uInt N){
    return create_mesh_full(N);
}
...
ComputingMesh create_mesh_full(uInt N){
    ...
    for(uInt ny = 2; ny < 5*N - 3; ny++){
        Scalar y = -0.25 + 2*dy * ny + dy + dy;
        for(uInt nx = 0; nx < 10*N - 3; nx++){
            Scalar x = nx * 2*dx + dx;
            if(x>1.0-h) break;
            internal_points.push_back({x,y});
...
ComputingMesh create_mesh_unstructure(uInt N){
    ...
    CGALMesh generator(0.25, h, h*0.866); // 设置长宽比为0.2，网格大小为h，厚度为0.5*h
