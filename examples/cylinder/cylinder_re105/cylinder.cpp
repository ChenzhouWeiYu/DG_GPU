    ...
    for(uInt save_step = 0; save_step < 400; save_step++){    
        ...
        **   sub_t 是当前的 Dt 间隔内的时间推进
        ...
        do{ 
            ...
            **   需要判断是否超过 Dt，超过了就截断
            \* ======================================================= */
	    std::array<Scalar,2000> history_residual;
	    history_residual.fill(0.0);
		...
		if (picard_iter>5 || total_time < 0.4){
		    ...
		    U_k_tmp = solver.SparseLU(U_k);
                ...
                logging("Re-discretize, calculate nonlinear residuals");
                {
                    ...
                    res_norm = std::sqrt(res_old.dot(res_old));
                    if(picard_iter<3) {
                        init_res_norm = res_norm;
                        prev_res_norm = res_norm;
                    }
		    history_residual[picard_iter] = res_norm;
                    rate_res_norm = res_norm/prev_res_norm;
		    if(picard_iter>5){
			uInt start = (picard_iter >= 5) ? picard_iter - 5 : 0;
			uInt end = picard_iter;
			auto [min_it, max_it] = std::minmax_element(history_residual.begin() + start, history_residual.begin() + end + 1);
			rate_res_norm = res_norm/(*max_it);
		    }
