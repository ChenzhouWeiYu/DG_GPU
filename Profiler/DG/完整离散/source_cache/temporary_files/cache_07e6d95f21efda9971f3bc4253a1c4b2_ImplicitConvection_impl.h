        ...
        for(uInt g=0; g<num_face_points; ++g) {
            ...
            for(uInt i = 0; i < Basis::NumBasis; ++i) {
                for(uInt j = 0; j < Basis::NumBasis; ++j) {
                    ...
                    // 计算系数
                    const Mat5x5& coeff_LL = dF_L * (phi_iL * phi_jL * weight);
                    const Mat5x5& coeff_LR = dF_L * (phi_iR * phi_jL * weight);
                    const Mat5x5& coeff_RL = dF_L * (phi_iL * phi_jR * weight);
                    const Mat5x5& coeff_RR = dF_R * (phi_iR * phi_jR * weight);
                    ...
                    for (uInt row = 0; row < 5; ++row) {
                        for (uInt col = 0; col < 5; ++col) {
                            ...
                            // LL 块
                            face_matrix_LL(global_row_LL, global_col_LL) = coeff_LL(row, col);

                            // LR 块
                            face_matrix_LR(global_row_LL, global_col_LL) = coeff_LR(row, col);

                            // RL 块
                            face_matrix_RL(global_row_LL, global_col_LL) = coeff_RL(row, col);

                            // RR 块
                            face_matrix_RR(global_row_LL, global_col_LL) = coeff_RR(row, col);
                        }
                    }
                }
            }
